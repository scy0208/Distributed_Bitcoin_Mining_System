...
object Client  {
  def main(args: Array[String]): Unit = {
    if (args.length < 3) {
      System.err.println("Client Usage: sbt run <server IP> <port> <worker> (<time>) ")
      System.exit(-1)
    }
    val remoteIP = args(0)
    val port=args(1)
    var time=0
    val numofworker=args(2).toInt
    if (args.length > 3){
      time =  args(3).toInt
    }
  ...
  val remotePath = "akka.tcp://serverSys@" + remoteIP + ":"+ port +"/user/server"
    ...
    val client = clientSystem.actorOf(Props(classOf[NodeClient], remoteServer,numofworker), name = "client")
...
class NodeClient(server: ActorSelection, numofworker:Int) extends Actor {
  var numofzero = 5
  ...
  val workerRouter = context.actorOf(
    Props(new Worker(self)).withRouter(RoundRobinRouter(numofworker)), name = "workerRouter")
